...
*
* hacked by   Argo Roots (argo@roots.ee)
*
*/
...
class Openid{
	...
	function __construct() {
		$CI =& get_instance();    
		$this->storePath = APPPATH .'../tmp/openid';
	...
	function authenticate($openId) {
		...
		if(is_string($consumer)) return $consumer;
		
		$authRequest = $consumer->begin($openId);
		
		// No auth request means we can't begin OpenID.
		if (!$authRequest) return 'Authentication error; not a valid OpenID.';
		...
		if ($this->ext_args != null) {
			foreach ($this->ext_args as $extensionArgument) {
				if (count($extensionArgument) == 3) $authRequest->addExtensionArg($extensionArgument[0], $extensionArgument[1], $extensionArgument[2]);
			}
		}
		...
		if ($authRequest->shouldSendRedirect()) {
			...
			if (Auth_OpenID::isFailure($form_html)) {
				...
				$page_contents = array(
					...
					"</title></head>",
					"<body style='display:none' onload='document.getElementById(\"".$form_id."\").submit()'>",
					$form_html,
					"</body></html>"
				);
	...
	function _getConsumer() {
		if (!file_exists($this->storePath) && !mkdir($this->storePath)) return 'Could not create the FileStore directory '. $this->storePath .'. Please check the effective permissions.';
